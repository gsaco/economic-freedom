...
- **Running variable:** signed market‑bloc vote margin (`running_var_vote`).
  - Implemented in `src/elections_parlgov.py` and `notebooks/11_construct_close_elections_rd_sample.py`.
- **Treatment:** `D = 1` if more‑market bloc wins (margin > 0).
- **EFW first stage:**
  - $
...
- **Direct‑effect bounds:** `notebooks/22_exclusion_sensitivity_bounds.py` computes $
\beta(h)
$ over a grid of $
\delta(h)
$ calibrated from pre‑trend RD estimates.
...
- **Weak first stage:** first‑stage tables + IV estimates reported only where `tau_EFW` non‑zero.
...
- **External validity:** ParlGov baseline treated as Tier‑1; global extensions documented as optional.

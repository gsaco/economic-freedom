...
|---|---|---|---|
| 0. Executive pitch + core claim discipline | `docs/04_methods_and_estimands.md` (claim ladder); paper integration pending | Doc review | DONE |
...
| 2. Running variable + treatment definition | `src/elections_parlgov.py`, `notebooks/11_construct_close_elections_rd_sample.py` | `make run` | DONE |
| 2. Orientation rules (market vs non‑market) | `src/elections_parlgov.py`, `docs/04_methods_and_estimands.md` | Doc + code review | DONE |
...
| 3. Elections/margins access (ParlGov baseline, CLEA/DPI/V‑Party fallbacks) | `notebooks/10_ingest_parlgov.py`, `docs/03_data_sources.md` | `make run` + doc | DONE |
| 3. Macro outcomes (WDI/PWT) | `notebooks/02_pull_worldbank.py`, `notebooks/03b_build_annual_panel.py` | `make run` | DONE |
| 3. Crisis data (Laeven‑Valencia) | `src/crisis.py`, `data/raw/crisis/laeven_valencia_2020.xlsx` | `make run` | DONE |
...
| 9. Novelty reference set | `references.bib`, `paper/main.tex` | grep for required citations | DONE |
